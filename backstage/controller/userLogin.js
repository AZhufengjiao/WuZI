...
module.exports.userRegister = async (ctx) => {
    ...
    const schema = Joi.object({
        ...
        password: Joi.string().min(6).max(20).pattern(new RegExp('^[a-zA-Z0-9]{3,30}$')).error(new Error('密码格式不正确')),
    ...
    // 调用mysql 注册用户 
    let res = await register(username, cryptoPassword(password + passwordSectet), mobile, email, role, state)
    console.log(res);
...
module.exports.login = async (ctx) => {
    ...
    // 在数据库中查询用户信息是否有
    const result = await userLogin(username, cryptoPassword(password + passwordSectet))
    let state = result[0]
    // 用户是否存在
    if (result[0] && state) {
        // 生成token
