...
module.exports.putMaterialsQuantity = async (ctx) => {
    ...
    const schema = Joi.object({
        materialName: Joi.string().min(1).max(20).required().error(new Error('物资名格式不正确')),
        quantity: Joi.number().integer().min(1).required().error(new Error('物资名格式不正确')),
        type: Joi.number().integer().valid(0, 1),
        username: Joi.string().min(1).max(20).required().error(new Error('用户名格式不正确')),
    });
...
module.exports.putMaterialsPrice = async (ctx) => {
    ...
    const schema = Joi.object({
        materialName: Joi.string().min(1).max(20).required().error(new Error('物资名格式不正确')),
        price: Joi.number().integer().min(1).required().error(new Error('物资名格式不正确')),
        type: Joi.number().integer().valid(0, 1),
        username: Joi.string().min(1).max(20).required().error(new Error('用户名格式不正确')),
    });
